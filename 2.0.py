...
def send_video_to_group(chat_id, video_path, title):
    # Send video to the group
    with open(video_path, "rb") as vf:
        msg = bot.send_video(GROUP_ID, vf, caption=f"{title}", supports_streaming=True)

    # Notify user with a direct link to the group
    bot.send_message(chat_id, f"✅ Your video '{title}' is posted in the group. Download it here:\nhttps://t.me/c/{str(GROUP_ID)[4:]}/{msg.message_id}")

    # Auto-delete the video from group after 1 hour (3600 seconds)
    threading.Timer(3600, lambda: bot.delete_message(GROUP_ID, msg.message_id)).start()
...
if size_mb > 50:
    ...
    ...
            ...
            # send video to group directly
            send_video_to_group(chat_id, final_path, title)
